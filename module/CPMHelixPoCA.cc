...
namespace
{
  ...
  bool finite_vector(const cpm::Vector3& value)
  {
    return std::isfinite(value.x) && std::isfinite(value.y) && std::isfinite(value.z);
  }

  double transverse_momentum(const cpm::Vector3& momentum)
  {
    return std::hypot(momentum.x, momentum.y);
  }

  double signed_omega(
      const cpm::HelixState& state,
      const cpm::HelixPoCAOptions& options,
  ...
  cpm::HelixPoCAResult line_fallback(
      const cpm::HelixState& state_a,
      const cpm::HelixState& state_b)
  {
    cpm::HelixPoCAResult result;
    const auto line_result = cpm::computeLocalLinePoCA(
        state_a.position,
        state_a.momentum,
        state_b.position,
        state_b.momentum);
