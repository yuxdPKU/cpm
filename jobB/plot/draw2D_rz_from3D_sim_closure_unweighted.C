...
void draw2D_rz_from3D_sim_closure_unweighted(int run=29, TString mode="acts", TString phiType="central")
{
gStyle->SetOptStat(0);
...
TString filename = Form("../output/sim_run%d_%s_unweighted/sim_run%d_%s_unweighted_maxdca0p2_B3_average_correction_histograms.root", run, mode.Data(), run, mode.Data());
...
gPad->RedrawAxis();
can->Update();
can->SaveAs(Form("figure/rz_NPRZ_from3D_%d_%s_unweighted_%s.pdf", run, tag.Data(), phiType.Data()));
delete can;

TFile* outfile = new TFile(Form("Rootfiles/hist_NPRZ_2Drz_%d_%s_unweighted_%s.root", run, tag.Data(), phiType.Data()), "recreate");
outfile->cd();
