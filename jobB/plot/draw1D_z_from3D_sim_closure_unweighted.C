...
void draw1D_z_from3D_sim_closure_unweighted(TString phiType="central")
{
gStyle->SetOptStat(0);
...
draw1Dmap(Form("../output/sim_run%d_truth_unweighted/sim_run%d_truth_unweighted_maxdca0p2_B3_average_correction_histograms.root", run, run), Form("CPM_sim_reco_truth"), selectR, selectPhi, hists_CPM_sim_truth, 4);
...
TFile* ofile_N = new TFile(Form("Rootfiles/hist_n_1Dz_R%d_unweighted_%s.root",(int)selectR,phiType.Data()),"recreate");
ofile_N->cd();
...
TFile* ofile_R = new TFile(Form("Rootfiles/hist_dr_1Dz_R%d_unweighted_%s.root",(int)selectR,phiType.Data()),"recreate");
ofile_R->cd();
...
TFile* ofile_P = new TFile(Form("Rootfiles/hist_dphi_1Dz_R%d_unweighted_%s.root",(int)selectR,phiType.Data()),"recreate");
ofile_P->cd();
...
TFile* ofile_Z = new TFile(Form("Rootfiles/hist_dz_1Dz_R%d_unweighted_%s.root",(int)selectR,phiType.Data()),"recreate");
ofile_Z->cd();
...
can->Update();
can->SaveAs(Form("figure/resid_vsZ_from3D_atR%d_unweighted_%s.pdf",(int)selectR,phiType.Data()));
...
legend->Draw();
can_leg->SaveAs(Form("figure/resid_vsZ_from3D_leg_unweighted_%s.pdf",phiType.Data()));
