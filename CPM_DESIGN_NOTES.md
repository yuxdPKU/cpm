...
- Following the CPM method note, B1 forms only same-charge pairs after applying the intra-voxel offset shift. B1 stores a curvature-proxy pair weight `1/(pt_i pt_j)`, equivalent to `(1/R_i)(1/R_j)` up to the common magnetic-field factor, and B2 uses this weight for the voxel-level crossing-point average.
