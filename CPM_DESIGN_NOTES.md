...
- The production path is record based: Job A writes compact `cpm_records`, B1 computes crossing points from the full file list, B2 accumulates voxel corrections, and B3 writes average-correction histograms. The older `CPMCorrectionContainer` route remains as a compatibility/debug path for files produced before this split.
...
- Production CPM track entries should store only the snapshot state needed for offline CPM: event/source identity, track id, charge, pT, cluster key, hitset key, TPC subsurfkey, voxel id, voxel center, cluster-to-voxel offset, and reference state position/momentum. More verbose debug fields such as corrected cluster coordinates, local state parameters, covariance, track quality, detector counters, and selection flags belong in the separate `PHCPMTpcCalibrationQA` helper.
...
- Added and later retired the Job A running `CPMCorrectionContainer` path. CPM correction calculation is now deferred to offline B1/B2 so sparse single-segment statistics do not discard useful records.
...
- Added `CPMAverageCorrectionReconstruction` and `CPM_ReconstructAverageCorrection.C` for the now-legacy container path.
