...
- The production path is record based: Job A writes compact `cpm_records`, and `CPM_ComputeAverageCorrection.C` computes crossing points from the full file list, accumulates voxel corrections, and writes average-correction histograms. Split-stage B1/B2/B3 macros are kept for QA and offline debugging.
...
- Production CPM `cpm_records` should be a `PHObject` container, following the `TpcSpaceChargeMatrixContainer` pattern. The concrete `CPMVoxelContainerv1` object is keyed by voxel and each voxel owns the compact track-state entries in that voxel. The compact entries store only the snapshot state needed for offline CPM: event/source identity, track id, charge, pT, cluster key, hitset key, TPC subsurfkey, voxel center, cluster-to-voxel offset, and reference state position/momentum. More verbose debug fields such as corrected cluster coordinates, local state parameters, covariance, track quality, detector counters, and selection flags belong in the flat `cpm_qa_records` tree written by `PHCPMTpcCalibrationQA`.
...
- Added first Job A flat ROOT persistence: `cpm_records` stores one compact ACTS-ready state/voxel record per row, and `cpm_metadata` stores grid/counter metadata.
...
- Added `CPM_ComputeAverageCorrection.C` as the compact production Job B macro. It performs the crossing calculation, voxel accumulation, and final histogram writing in one file, leaving detailed intermediate products to the QA macros.
