...
- Decision: no suitable ready-made ideal two-helix PoCA implementation was found. CPM now has an initial CPM-local numerical ideal-helix PoCA module, `CPMHelixPoCA`, with an interface narrow enough to upstream later into coresoftware if validation is successful. B1 uses the helix solver by default, while the local line-line solver remains available through an explicit `line` option.
...
- The residual cuts in the legacy workflow include `setMinPt(0.5)`, `requireCrossing(false)`, `requireCM(true)`, `setPCAzcut(10)`, `setEtacut(0.25)`, `setMaxTrackAlpha(0.6)`, `setMaxTrackBeta(1.5)`, `setMaxTrackResidualDrphi(2)`, `setMaxTrackResidualDz(5)`, `setMinRPhiErr(0.005)`, and `setMinZErr(0.01)`. CPM keeps the track/TPOT requirements relevant to the crossing method, but does not apply the legacy central-membrane requirement in Job A.
...
- CPM should reuse the same first-pass selection logic where possible: pruned seed map `PrunedSvtxTrackSeedContainer`, `pT > 0.5`, at least one TPOT cluster/state, and the crossing requirement setting. The legacy central-membrane requirement is not part of the CPM Job A selection.
...
- selection flags: crossing availability, TPOT requirement result, and geometry sanity checks.
...
- `process_event()` collects compact per-cluster/per-track entries into per-voxel charge queues and fills a mergeable correction container as soon as both charge queues reach the running threshold. At that point CPM pairs all currently queued positive tracks with all currently queued negative tracks in the voxel, then clears that voxel queue.
- The production path should align with `TpcSpaceChargeMatrixContainer`: Job A writes additive per-voxel correction sums and entries, and Job B merges those containers before writing average-correction histograms. The B2/B3 production work now lives in `CPMAverageCorrectionReconstruction`, analogous to `TpcSpaceChargeMatrixInversion`, with `CPM_ReconstructAverageCorrection.C` as the thin macro entry point. The older record-based B0/B1/B2 path remains useful for diagnostics, event rehydration studies, and solver comparisons and is exposed through a single QA driver macro.
...
- CPM should not write `TpcSpaceChargeMatrixContainer` directly, but it should use the same mergeable-container semantics. Its merged output should be correction-component histograms compatible with `TpcDistortionCorrectionContainer`, plus QA histograms.
- Histogram naming and coordinate conventions should match the current average correction files: `hIntDistortionP_{negz,posz}`, `hIntDistortionR_{negz,posz}`, `hIntDistortionZ_{negz,posz}`, and `hentries_{negz,posz}` after splitting/guard-bin handling, with the existing `[0, 2pi]` phi-axis convention. For average corrections, `hIntDistortionP` is a phi-angle correction in radians because `G4TPC::USE_PHI_AS_RAD_AVERAGE_CORRECTIONS` defaults to true; the CPM final histogram writer therefore uses `mean_delta_phi`, while `mean_delta_rphi` remains a QA quantity in B2.
...
- Following the 2026-05-09 expert feedback, B1 now forms opposite-charge pairs after applying the intra-voxel offset shift. B1 keeps one closest-to-voxel-center record per unique track, then processes the selected records in deterministic hash-ordered batches with a configurable per-charge batch limit. The raw per-voxel safety skip is disabled by default (`--b1-max-records 0`). The current batch default is 10 records per charge sign per batch; 0 keeps one unlimited full-voxel batch for diagnostic studies. This avoids throwing away all records beyond an arbitrary first-N cap while keeping the pair loop bounded. B1 still stores a curvature-proxy pair weight `1/(pt_i pt_j)`, equivalent to `(1/R_i)(1/R_j)` up to the common magnetic-field factor. B1 now writes `cpm_b1_batch_corrections`, with accepted pair counts, total weights, effective entries, unweighted sums, weighted sums, sum-of-squares, and DCA QA for each batch. B2 can read these batch sums in `auto`/`batches` mode and produce pair-equivalent weighted or unweighted voxel averages without relying on persistent pair rows.
...
- CPM track entries should store enough snapshot state to allow either local-line PoCA or later ACTS propagation/minimization: cluster key, hitset key, TPC subsurfkey, distorted cluster coordinate, voxel offset, charge, reference state local/global position, momentum, full state covariance, pathlength, and track quality. The workflow does not persist a separate ACTS surface snapshot; surfaces can be recovered from geometry with `hitsetkey + subsurfkey` if needed.
...
- Added first Job A flat ROOT persistence: `cpm_records` can optionally store one ACTS-ready state/voxel QA record per row, and `cpm_metadata` stores grid/counter metadata. This record tree is disabled by default now that the mergeable correction container is the production output.
...
- Added the first CPM-local ideal-helix PoCA helper, `CPMHelixPoCA`, plus framework-independent tests. B1 now runs the helix solver by default and can still run the local-line solver by option, so the two methods can be compared on identical Job A input. The helix solver currently uses a configurable uniform `Bz` estimate, defaulting to `1.4 T`, as a first-pass approximation while the rest of the workflow is developed.
- Added B1 batch-level running sums and B2 batch-input accumulation. B2 `auto` mode prefers `cpm_b1_batch_corrections`, while `pairs` mode remains available for pair-level QA and backwards compatibility.
- Added a mergeable `CPMCorrectionContainer` filled inside Job A by flushing running opposite-charge voxel queues. This is now the matrix-inversion-aligned production path: the module-backed Job B reconstruction merges the containers and can write either plain or pair-weighted voxel averages.
- Renamed the B1 driver macro to `CPM_QA_B1_ComputePoCA.C` and the default B1 intermediate output to `*_B1_poca.root`, because B1 is now solver-agnostic and defaults to the helix PoCA method rather than the original local line-line method.
- Added `CPMAverageCorrectionReconstruction` and `CPM_ReconstructAverageCorrection.C` so the production Job B path now merges `CPMCorrectionContainer` inputs, calculates plain or weighted average corrections, writes the voxel QA TTree, and writes the final guarded average-correction histograms from one module-backed entry point.
- Added `CPM_QA_RunOfflineDiagnostics.C` as the single user-facing macro for the optional `cpm_records` path. It runs the record-based B0/B1/B2/B3 diagnostic sequence while keeping separate stage ROOT files for offline debugging.
