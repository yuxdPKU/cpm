...
- Following the 2026-05-09 expert feedback, B1 now forms opposite-charge pairs after applying the intra-voxel offset shift. B1 keeps one closest-to-voxel-center record per unique track, then processes the selected records in deterministic hash-ordered batches with a configurable per-charge batch limit. This avoids throwing away all records beyond an arbitrary first-N cap while keeping the pair loop bounded. B1 still stores a curvature-proxy pair weight `1/(pt_i pt_j)`, equivalent to `(1/R_i)(1/R_j)` up to the common magnetic-field factor. B2 can now either use these pair weights for the voxel-level crossing-point average or run in an unweighted simple-average mode for cross-checks.
...
- Added the first CPM-local ideal-helix PoCA helper, `CPMHelixPoCA`, plus framework-independent tests. B1 now runs the helix solver by default and can still run the local-line solver by option, so the two methods can be compared on identical Job A input.
