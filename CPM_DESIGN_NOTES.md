...
- Decision: no suitable ready-made ideal two-helix PoCA implementation was found. CPM now has an initial CPM-local numerical ideal-helix PoCA implementation in `CPMReconstructionHelper`, with an interface narrow enough to upstream later into coresoftware if validation is successful. B1 uses the helix solver by default, while the local line-line solver remains available through an explicit `line` option.
...
- `process_event()` collects compact per-cluster/per-track entries only. CPM pair construction and averaging are intentionally deferred to offline Job B so many DST/segment outputs can be combined before applying voxel statistics.
- The production path is record based: Job A writes compact `CPMVoxelContainer`, and the module-level `CPMAverageCorrectionReconstruction` computes crossing points from the full file list, accumulates voxel corrections, and writes average-correction histograms. `CPM_ComputeAverageCorrection.C` is only the ROOT macro entry point. Split-stage B1/B2/B3 macros are kept for QA and offline debugging.
...
- CPM should not write `TpcSpaceChargeMatrixContainer` directly. Its offline merged output should be correction-component histograms compatible with `TpcDistortionCorrectionContainer`, plus QA histograms.
...
- Following the 2026-05-09 expert feedback, CPM forms opposite-charge pairs after applying the intra-voxel offset shift. It keeps one closest-to-voxel-center record per unique track, then processes the selected records in deterministic hash-ordered batches with a configurable per-charge batch limit. The raw per-voxel safety skip is disabled by default (`--max-records 0`). The current batch default is 10 records per charge sign per batch; 0 keeps one unlimited full-voxel batch for diagnostic studies. This avoids throwing away all records beyond an arbitrary first-N cap while keeping the pair loop bounded. CPM stores a curvature-proxy pair weight `1/(pt_i pt_j)`, equivalent to `(1/R_i)(1/R_j)` up to the common magnetic-field factor. In production these accepted pairs are accumulated directly into voxel averages; QA B1/B2 can still persist batch and voxel trees for offline debugging.
...
- Production CPM `CPMVoxelContainer` is a `PHObject` container, following the `TpcSpaceChargeMatrixContainer` pattern. The concrete `CPMVoxelContainerv1` object is keyed by voxel and each voxel owns the compact track-state entries in that voxel. The compact entries store only the snapshot state needed for offline CPM: event/source identity, track id, charge, pT, cluster key, hitset key, TPC subsurfkey, voxel center, cluster-to-voxel offset, and reference state position/momentum. More verbose debug fields such as corrected cluster coordinates, local state parameters, covariance, track quality, detector counters, and selection flags belong in the flat `cpm_qa_records` tree written by `PHCPMTpcCalibrationQA`.
...
- Added first Job A compact ROOT persistence: `CPMVoxelContainer` stores compact ACTS-ready state/voxel records grouped by voxel, and `cpm_metadata` stores grid/counter metadata.
- Drafted the first CPM Job B0 event-index skeleton. It scans `CPMVoxelContainer`, sorts/deduplicates event references, and writes `cpm_event_requests` plus `cpm_object_requests` for a later sequential mini-DST rehydration pass.
...
- Added the first CPM-local ideal-helix PoCA implementation in `CPMReconstructionHelper`, plus framework-independent tests. B1 now runs the helix solver by default and can still run the local-line solver by option, so the two methods can be compared on identical Job A input. The helix solver currently uses a configurable uniform `Bz` estimate, defaulting to `1.4 T`, as a first-pass approximation while the rest of the workflow is developed.
- Added B1 batch-level running sums and B2 batch-input accumulation for the QA split-stage path. B2 `auto` mode prefers `cpm_b1_batch_corrections`, while `pairs` mode remains available for pair-level QA and backwards compatibility.
- Removed the retired Job A running correction-container path. CPM correction calculation is now deferred to offline Job B so sparse single-segment statistics do not discard useful records.
...
- Added `CPM_QA_RunOfflineDiagnostics.C` as a single user-facing macro for the record-based path. It runs optional B0 plus B1/B2/B3 while keeping separate stage ROOT files for offline debugging.
- Added `CPMAverageCorrectionReconstruction` as the module-level production Job B calculation driver. `CPM_ComputeAverageCorrection.C` now only configures this driver, loads input files, and saves the final output, leaving detailed intermediate products to the QA macros.
