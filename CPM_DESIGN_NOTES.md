...
- Following the 2026-05-09 expert feedback, B1 now forms opposite-charge pairs after applying the intra-voxel offset shift. B1 still stores a curvature-proxy pair weight `1/(pt_i pt_j)`, equivalent to `(1/R_i)(1/R_j)` up to the common magnetic-field factor. B2 can now either use these pair weights for the voxel-level crossing-point average or run in an unweighted simple-average mode for cross-checks.
